...
public class BaseDao<E,I> implements IDao<E,I>{
    ...
    private Set<String> populateTimestamp(User user, E entity, Set<String> columns){
        if (user != null){
            ...
            if (entity instanceof UserScopeEntity){
                ((UserScopeEntity)entity).setUserId(userId);
            }
        }
    ...
    protected SelectQuery<E> listSelectBuilder(User user, Condition filter, String... orderBy) {
        String[] o = getOrderByOrDefault(orderBy);
        return Query.select(entityClass).where(filter).orderBy(getOrderByOrDefault(orderBy));
    }
