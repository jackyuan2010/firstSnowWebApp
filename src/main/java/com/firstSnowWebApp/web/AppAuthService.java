...
import com.firstSnowWebApp.AppException;
import com.firstSnowWebApp.ErrorType;
import com.firstSnowWebApp.entity.*;
import com.firstSnowWebApp.dao.UserDao;
import com.firstSnowWebApp.service.PasswordService;
import com.firstSnowWebApp.access.AccessManager;
...
public class AppAuthService implements AuthRequest{
    ...
    @WebPost("/register")
    public WebResponse register(@WebParam("username") String username, @WebParam("password")String clearPwd,
        ...
        if (Strings.isNullOrEmpty(username)) {
            ...
            return WebResponse.fail(new AppException(Error.CANNOT_REGISTER_EMPTY_NAME));
        }
        if (Strings.isNullOrEmpty(clearPwd)){
            //throw new AppException(Error.CANNOT_REGISTER_WITH_EMPTY_PASSWORD);
            return WebResponse.fail(new AppException(Error.CANNOT_REGISTER_WITH_EMPTY_PASSWORD));
        }
        if (!clearPwd.equals(pwdRepeat)) {
            //throw new AppException(Error.REPEAT_PASSWORD_NO_MATCH);
            return WebResponse.fail(new AppException(Error.REPEAT_PASSWORD_NO_MATCH));
        }
        if (userDao.getByUsername(username) != null){
            //throw new AppException(Error.CANNOT_REGISTER_USERNAME_ALREADY_EXIST);
            return WebResponse.fail(new AppException(Error.CANNOT_REGISTER_USERNAME_ALREADY_EXIST));
        }
    ...
    @WebPost("/login")
    public WebResponse login(@WebParam("username")String username, @WebParam("password")String clearPwd, RequestContext rc){
        User user = userDao.getByUsername(username);

        // check if user exist
        if (user == null){
            //throw new AppException(Error.WRONG_CREDENTIAL);
            return WebResponse.fail(new AppException(Error.WRONG_CREDENTIAL));
        }
    ...
    static String buildUToken(User user){
        return Hashing.sha1().hashString(user.getUsername() + TOKEN_SALT + user.getPassword(), Charsets.UTF_8).toString();
    }
}
